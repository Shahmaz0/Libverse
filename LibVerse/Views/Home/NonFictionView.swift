//
//  NonFictionView.swift
//  LibVerse
//
//  Created by Swift AI on 01/05/25.
//

import Foundation
import SwiftUI

// MARK: - Non-Fiction View
struct NonFictionView: View {
                ...
                ScrollView {
                    LazyVGrid(columns: columns, spacing: 24) {
                        ForEach(filteredBooks) { book in
                            NonFictionBookGridItem(book: book)
    ...
    private func fetchBooks() {
        ...
        Task {
            do {
                print("Fetching Non-Fiction books from Supabase...")
                
                let response: [LibraryBook] = try await SupabaseManager.shared.client
                    .from("Books")
                    .select("*")
                    .eq("genre", value: "Non-Fiction")
                    .order("title")
                    .execute()
                    .value
...
// MARK: - Book Grid Item for Non-Fiction
struct NonFictionBookGridItem: View {
